...
const BusinessFutureForm = ({setOn}) => {
    ...
    <div className={styles.popup}>
        <div className={styles.head}>
            ...
            <img className={styles.close} onClick={() => setOn(false)} src={img1} alt='Close'/>
        </div>
        <form action="" className={styles.form}>
            ...
            <div className={styles.projectIdea}>
                ...
                <label onClick={() => setActive(true)} for="yes">Есть</label>

                <input id="no" type="radio" name="question" value="2" checked/>
                <label onClick={() => setActive(false)} for="no">Нет</label>
            </div>
            {active && <Textarea/>}
